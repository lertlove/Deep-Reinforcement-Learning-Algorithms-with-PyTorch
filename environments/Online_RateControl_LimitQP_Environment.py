...
NUM_QP_LEVELS = 7  # <- 7[-3,3] from inital qp //default is 52
MAX_NUM_CTUS = 100
...
class Online_RateControl_Environment(gym.Env):
    ...
    def get_state_size(self):
        ...
        # return 10 because Bank added frame_level_qp
        return 10
    ...
    def request_action(self, message, state=None):
        ...
        # exact qp value would be frame level qp + action( 7 level which is[-3,3])
        # and check exact qp 0 < exact qp < 52

        exact_qp_value = action - 3 + self.frame_level_qp
    ...
    def setupTargetBits(self, message):
        ...
        self.frame_level_qp = message["sliceQP"]
    ...
    def reset(self):
        ...
        ctu_variance = (
            self.ctuVariants[self.current_ctu] / self.maxVariant
            if self.maxVariant != 0
            else 0
        )  # compute first ctu variance
        percent_ctu_area = (ctu_width * ctu_height) / (self.total_area)

        avg_remaining_ctu_variants = (
            np.mean(self.ctuVariants[self.current_ctu : self.total_num_ctus])
            / self.maxVariant
            if self.maxVariant != 0
            else 0
        )
        ...
        # self.episodeVariables = np.array([self.total_target_bit,self.total_num_ctus, self.total_area])#, self.frame_level_qp]) # add frame level initial qp
        self.episodeVariables = np.array(
            [
                self.total_target_bit,
                self.total_num_ctus,
                self.total_area,
                self.frame_level_qp,
            ]
        )  # add frame level initial qp

        self.stateVariables = np.array(
            [
                self.percent_bit_balance,
                percent_remaining_ctu,
                ctu_variance,
                percent_ctu_area,
                avg_remaining_ctu_variants,
                self.percent_remaining_area,
            ]
        )
    ...
    def done_action(self):
        ...
        if self.current_ctu < self.total_num_ctus:
            percent_remaining_ctu = (
                self.total_num_ctus - self.current_ctu
            ) / self.total_num_ctus
            ctu_height, ctu_width = self.ctuShapes[self.current_ctu]
            ctu_variance = (
                self.ctuVariants[self.current_ctu] / self.maxVariant
